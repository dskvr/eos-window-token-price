var Web3 = require('web3'), web3, start_pos = 0, num_windows = 30, max_num_windows = 350, loading_percentage = 0, loop_buffer=100, usd_per_eth, Contract, EOSCrowdsale, window_today;
...
var iterator = function(stop){
  //console.log('iter_pos: '+iterator_position+', num_windows:'+num_windows);
...
function formatResult(day,today)
{
  ...
  var eos_per_eth = eos_per_window / eth_contributed;
  ...
  results += '<td class="eospereth">'+eos_per_eth.toFixed(2)+'</td>';
  results += '<td class="rate">$' + usd_per_eos.toFixed(2) + '</td>';
  results += '<td class="rate">' + eth_per_eos.toFixed(6) + '</td>';
  results += '</tr>';
  return results;
}
...
$(function(){    
  ...
  if(typeof cookie == "object") {
    ...
    $('form').find('input.start-from').val(cookie.start);
  ...
  $('#set-node').submit(function(e){
    ...
    var start = $(this).find('input.start-from').val();    
    ...
    if((parseInt(start) + parseInt(days)) > max_num_windows){
      start = max_num_windows - days;
    }    
    //
    Cookies.set('eostracker', {'node':node.trim(),'days':days.trim(),'start':start.trim()});
    //
    start = parseInt(start);
    iterator_position = start;
    num_windows = parseInt(days) + start;
    
    //console.log('start: ' + start + ', days:' + days);
